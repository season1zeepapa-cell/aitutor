...
#### 학습 탭 (QuizTab) ✅
- [x] 카테고리/시험 필터 + 페이지네이션
- [x] 플래시카드 UI (펼치기/접기, 선택지, 정답/오답)
- [x] 문제 이미지 토글 (원본 이미지 보기 버튼)
- [x] 기본 해설 표시 (HTML 렌더링 지원)
- [x] AI 해설 3개 프로바이더 (Gemini/OpenAI/Claude SSE 스트리밍)
- [x] AI 해설 수동 저장/삭제/저장본 조회
- [x] AI 해설 이미지 포함 옵션 + 추가 지시사항 입력
- [x] AI 해설 트래킹 패널 (요청/프롬프트/응답/저장 추적)
- [x] 메모 CRUD + 첨부파일 (업로드/다운로드/삭제)
- [x] 법령 키워드 자동 링크 (「법령명」 클릭 → 법제처/AI법령정보)
- [x] 법령검색 패널 + AI 법령정보 체크박스
- [x] 내용 복사 버튼
- [x] 이미지 확대 모달

#### 문제관리 탭 (ManageTab) ✅
- [x] 문제 목록 + 카테고리/시험 필터
- [x] 문제 추가/수정 모달 (QuestionForm)
- [x] 문제 삭제
- [x] 체크박스 전체선택 + 일괄 삭제
- [x] 일괄 과목 지정
...
- [x] 소스 시험 선택 + 문제 로드
- [x] 칸반 3단계 (대상조회 → 문제이관 → 해설생성 및 완료)
- [x] 카테고리 선택 + 시험이름 입력
- [x] 체크박스 + 전체선택
- [x] 문제이관 / 해설생성 / 소스삭제 버튼
- [x] LLM 프로바이더 선택
- [x] 진행 상태 바 + 처리 로그 (복사/초기화)

#### 설정 탭 (SettingsTab) ✅
- [x] 카테고리 CRUD (추가/수정/삭제)
- [x] 과목 CRUD (추가/수정/삭제)
- [x] 시험별 카테고리 지정
- [x] LLM 설정 패널 (모델 선택, Temperature, MaxTokens, Thinking, Reasoning)
- [x] 회원관리 (목록/권한 토글/삭제)

#### 공통 ✅
- [x] 토스트 알림 (ToastProvider + useToast)
- [x] 이미지 확대 모달 (ImageModal)
- [x] 에러 바운더리 (ErrorBoundary)
- [x] 다크모드 (useTheme + CSS 변수)
- [x] 하단 네비게이션 (BottomNav)
- [x] 인증 (로그인/회원가입/로그아웃)
- [x] SSE 스트리밍 공통 훅 (useSSE)
