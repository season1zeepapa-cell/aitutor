...
//   - 건너뛰기 / 이전 / 다음 / 완료
//
// v5 변경점 (2026-07-04, REBUILD91):
//   - mcq 유형 폐지 → 이론 객관식(objective) 일원화 반영 (REBUILD84)
//   - 6유형 2,852문항 체계(코드식별 1,359·객관식 562·서술형 562·진단 138·단답형 138·복합 93)로 수치 갱신
//   - 예시코드 라이브러리 학습(읽기 진도·🎓 블라인드 판별·약점 드릴 연결, REBUILD86) 단계 신설
//   - 모의고사 구성 갱신: 이론 60분(객관식 20+단답형 10) / 실기 100분(진단 13+복합 2+코드식별 5)
//   - 오답노트 단계 추가, 버전 키 v4 → v5 (기존 사용자 재안내)
//
// v2 (2026-04-24): 일반 기출 트랙 추가 + 트랙 선택 초기 화면
// v3: 트랙명 '기출문제 학습' 으로 일반화 (3개 자격증 지원)
// v4 (2026-04-25): KISA 단답형 유형·3종 드릴·모의고사 구성 반영
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';

const STORAGE_KEY = 'aitutor-tutorial-seen';
const VERSION = 'v5';  // v5 — 6유형 체계·라이브러리 학습 모드 반영 (2026-07-04)
...
const KISA_STEPS = [
  {
    emoji: '🛡️',
    title: 'KISA 진단원 이수시험',
    body: 'SW 보안약점 진단원 이수시험은 이론 60분 30문항 + 실기 100분(종합 70점 합격)으로 구성됩니다. 이 앱은 6가지 문제 유형 총 2,852문항 + 69개 학습 챕터 + 공식문서 5종 지식 라이브러리로 완전 대비를 지원해요.',
  ...
  },
  {
    emoji: '📐',
    title: '먼저, 단계를 구분하세요',
    body: '설계단계(설계 요구사항, 20항목)와 구현단계(취약 코드 진단, 49항목)로 구분됩니다. 대시보드 하단 탭으로 각 단계 카테고리를 확인하세요. 이론 객관식·단순서술형·단답형은 양쪽 단계 모두 포함됩니다.',
  ...
  },
  {
    emoji: '📖',
    title: '학습 자료부터 시작',
    body: '69개 챕터의 정의 · 원인 · 대응 원칙 · 취약/안전 코드 비교를 먼저 읽으세요. 설계↔구현 연관 챕터가 서로 연결돼 있고, 챕터 하단에서 그 약점의 객관식·단답형·코드식별 드릴을 바로 시작할 수 있어요.',
  ...
  },
  {
    emoji: '🎯',
    title: '6가지 문제 유형',
    body: '대시보드에서 유형별 드릴을 선택할 수 있어요:\n\n📝 이론 객관식 (562) — 코드/설명 지문 + 4~5지선다\n🖊️ 단순서술형 (562) — 정·오탐 분석 서술, 키워드 채점\n✍️ 단답형 (138) — 빈칸 채우기, 69챕터 × 2\n🧪 실기 진단 (138) — 취약 코드 4단계 진단\n📋 복합서술형 (93) — 산출물 검토 → 진단보고서, 전 챕터\n💻 코드식별 (1,359) — 코드 보고 약점·안전/취약 판별',
    highlight: '대시보드 드릴 카드들',
    cta: { label: '이론 객관식 시작', path: '/kisa/drill-config?type=objective' },
  },
  {
    emoji: '✍️',
    title: '단답형 풀이 방법',
  ...
  {
    emoji: '💾',
    title: '예시코드 라이브러리 = 조회 + 학습',
    body: '공식문서 5종의 취약(❌)/안전(✅) 코드를 원문 그대로 조회하고, 항목을 펼치면 읽음으로 기록돼 진도·이어보기가 돼요.\n\n🎓 학습 모드로 전환하면 라벨을 가린 코드 1,359장을 보고 취약/안전을 판별하는 블라인드 훈련이 가능하고, 헷갈린 코드는 한 번에 복습 큐로 보낼 수 있습니다.',
    highlight: '대시보드 [예시코드 라이브러리] → 우상단 [🎓 학습] 토글',
    cta: { label: '라이브러리 열기', path: '/kisa/code-library' },
  },
  {
    emoji: '📝',
    title: '정답 해설 + AI 추가 해설',
    body: '제출 후 문제 유형별 결과 모달이 나타납니다:\n\n📝 객관식·코드식별: 선택 vs 정답 + 선지별 해설\n✍️ 단답형: 빈칸별 입력/정답 카드 + 정답률 배지\n🧪 실기·서술형: 라인·근거·키워드(rubric) 채점\n\n🤖 Gemini / Claude / OpenAI 버튼으로 유형별 맞춤 AI 추가 해설을 받을 수 있어요. 저장된 해설은 ✓ 뱃지로 표시되며 즉시 불러옵니다.',
  ...
  },
  {
    emoji: '⏱️',
    title: '실전 모의고사',
    body: '이론 60분(객관식 20 + 단답형 10 = 30문항) / 실기 100분(진단 13 + 복합 2 + 코드식별 5 = 20문항) / 전체 180분(50문항) 모드를 선택할 수 있습니다. 매번 문항 풀에서 랜덤 조합되고, 30초마다 자동저장되므로 네트워크가 끊겨도 안전해요.',
  ...
  },
  {
    emoji: '🔁',
    title: 'SRS 복습 + 오답 노트',
    body: '자가평가 [다시/어려움/괜찮음/쉬움]에 따라 SM-2 알고리즘이 복습일을 정해 자동 재출제합니다. 복습 화면에서 유형별·약점 그룹별로 골라 복습할 수 있어요.\n\n📝 오답 노트에는 70점 미만 문항이 모이고, 카드에서 내 답·정답·해설 확인 후 그 약점만 바로 다시 풀 수 있습니다.',
    highlight: '대시보드 [복습 (SRS)] · [오답 노트]',
    cta: { label: '복습 열기', path: '/kisa/review' },
  },
  {
    emoji: '📊',
    title: '통계 대시보드',
    body: '약점 분류별 정답률 / 최근 7일 학습량 / 향후 7일 복습 예정을 차트로 확인할 수 있어요.',
  ...
  },
  {
    emoji: '🎉',
    title: '준비 완료!',
    body: '추천 순서: 📖 학습 자료 → 📝 이론 객관식 → 💾 라이브러리 학습 모드 → 🧪 실기 드릴 → ⏱️ 실전 모의.\n\n언제든 KISA 대시보드 [❓ 가이드] 버튼으로 다시 볼 수 있어요.',
    cta: { label: 'KISA 대시보드', path: '/kisa' },
  },
];
          ...
          <div className="px-6 pb-5 text-center">
            ...
            <div className="space-y-2">
              ...
              <TrackButton
                emoji="🛡️"
                title="KISA 진단원 이수시험"
                desc="SW 보안약점 6유형 2,852문항 · 69챕터 · 5문서 라이브러리"
