...
export default function ServerAiTester() {
  const [modelKey, setModelKey] = useState('e2b-gguf');
  ...
  return (
    <div className="max-w-md mx-auto p-4 space-y-4">
      <header className="flex items-center justify-between">
        <h1 className="text-lg font-bold text-text">⚡ 서버 추론 GGUF (llama.cpp)</h1>
        <a href="/" className="text-xs text-primary hover:underline">← 홈</a>
      </header>

      {/* 안내 배너 */}
      <div className="rounded-lg border border-purple-200 dark:border-purple-800 bg-purple-50 dark:bg-purple-900/30 px-3 py-2 text-[11px] text-purple-900 dark:text-purple-200 leading-relaxed">
        ⚡ <b>Lambda + llama.cpp + GGUF Q4_K_M</b> — ONNX 12단계 트러블 회피.
        CPU 추론 최적화. 외부 회사 X, 우리 AWS 안에서만 처리.
      </div>
...
{promptText}
      ...
      <p className="text-[11px] text-text-secondary text-center pt-4">
        REBUILD21 — Python Lambda Container + llama-cpp-python + GGUF + LWA
      </p>
    </div>
  );
}
