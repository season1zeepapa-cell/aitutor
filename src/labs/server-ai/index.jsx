...
//
// 가드: /api/config 의 lab_server_ai_enabled === true 일 때만 ServerAiTester 마운트
...
export default function ServerAiLab() {
  ...
  useEffect(() => {
    fetch('/api/config', { credentials: 'include' })
      ...
      .then(data => setEnabled(data?.lab_server_ai_enabled === true))
    ...
    return (
      <div className="max-w-md mx-auto p-6 mt-12 text-center">
        ...
        <p className="text-[11px] text-text-secondary mt-2">
          관리자가 <code className="px-1 bg-card-bg rounded">lab_server_ai_enabled</code> 설정을 켜야 표시됩니다.
        </p>
      </div>
    );
  }
